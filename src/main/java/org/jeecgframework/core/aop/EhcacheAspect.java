...
public class EhcacheAspect {
	...
	private static Cache SYSTEM_BASE_CACHE = CacheManager.getInstance().getCache(GlobalConstants.SYSTEM_BASE_CACHE);
	private static Cache SYS_AUTH_CACHE = CacheManager.getInstance().getCache(GlobalConstants.SYS_AUTH_CACHE);
	...
	public Object aroundLogCalls(ProceedingJoinPoint joinPoint,Ehcache ehcache)throws Throwable {
		...
		if(oConvertUtils.isNotEmpty(ehcache.cacheName()) && GlobalConstants.SYS_AUTH_CACHE.equals(ehcache.cacheName())){
			//自定义缓存对象
