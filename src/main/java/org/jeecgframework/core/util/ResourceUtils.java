...
public class ResourceUtils {
     ...
     * 当前登录用户
     */
    public static final String LOCAL_CLIENT_USER = "LOCAL_CLIENT_USER";
     ...
     * 国际化缓存key
     */
    public static final String MULTI_LANG_FOREVER_CACHE_KEY = "forever_cache_muti_langs";
    ...
    public static TSTypegroup getCacheTypeGroup(String typeGroupCode) {
        ...
        if (obj != null) {
            ...
            TSTypegroup result = mp.get(typeGroupCode);
            logger.debug("-----------从缓存获取字典组-----typeGroupCode: [{}]", typeGroupCode);
            return result;
        }
        return null;
    }
    ...
    public static List<TSType> getCacheTypes(String typeGroupCode) {
        ...
        if (obj != null) {
            ...
            List<TSType> result = mp.get(typeGroupCode);
            logger.debug("-----------从缓存获取字典-----typeGroupCode: [{}]", typeGroupCode);
            return result;
        }
        return null;
    }
     ...
     * @return
     */
    public static String getMultiLan(String key) {
        Object obj = cacheService.get(CacheServiceI.FOREVER_CACHE, MULTI_LANG_FOREVER_CACHE_KEY);
        if (obj != null) {
            Map<String, String> ls = (Map<String, String>) obj;
            String result = ls.get(key);
            logger.debug("-----------从缓存获取国际化-----key：[{}] , result[{}]", key, result);
            return result;
        }
        return null;
    }
    ...
    public static Map<String, DynamicDataSourceEntity> getCacheAllDynamicDataSourceEntity() {
        ...
        if (obj != null) {
            ...
            logger.debug("-----------从缓存获取动态数据源配置-------getCacheAllDynamicDataSourceEntity--------size：[{}] ", ls.size());
            return ls;
        }
        return null;
    }
    ...
    public static DynamicDataSourceEntity getCacheDynamicDataSourceEntity(String dbKey) {
        ...
        if (obj != null) {
            Map<String, DynamicDataSourceEntity> ls = (Map<String, DynamicDataSourceEntity>) obj;
            DynamicDataSourceEntity result = ls.get(dbKey);
            logger.debug("-----------从缓存获取动态数据源配置----getCacheDynamicDataSourceEntity-----dbKey：[{}]", dbKey);
            return result;
        }
        return null;
    }
     ...
     * 缓存系统图标【缓存】
     */
    public static Map<String, TSIcon> allTSIcons = new HashMap<>();
    ...
    public static final TSUser getSessionUser() {
        ...
        if (clientManager.getClient(session.getId()) != null) {
            ...
            TSUser u = (TSUser) session.getAttribute(ResourceUtils.LOCAL_CLIENT_USER);
     ...
     * @return
     */
    public static String getJgAuthRequestPath(HttpServletRequest request) {
    ...
    public static String getUserAccount() {
        return getUserSystemData(DataBaseConstant.SYS_USER_CODE);
    }
    ...
    public static String getUserName() {
        return getUserSystemData(DataBaseConstant.SYS_USER_NAME);
    }
    ...
    public static String getUserCompanyCode() {
        return getUserSystemData(DataBaseConstant.SYS_COMPANY_CODE);
    }
    ...
    public static String getUserOrgCode() {
        return getUserSystemData(DataBaseConstant.SYS_ORG_CODE);
    }
     ...
     * @param ruleValue Session 中的值
     * @return
     */
    public static String converRuleValue(String ruleValue) {
        String value = ResourceUtils.getSessionData(ruleValue);
        if (StringUtil.isEmpty(value)) {
            value = ResourceUtils.getUserSystemData(ruleValue);
        }
