...
public class BlackListController extends BaseController {
	...
	@Autowired
	private BlackListService tsBlackListService;
	@Autowired
	...
	public void datagrid(BlackListEntity tsBlackList, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
	...
	@ResponseBody
	public AjaxJson doDel(BlackListEntity tsBlackList, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		tsBlackList = systemService.getById(BlackListEntity.class, tsBlackList.getId());
		message = "黑名单删除成功";
		try{
	...
	public AjaxJson doBatchDel(String ids,HttpServletRequest request){
		...
		try{
			for(String id:ids.split(",")){
				BlackListEntity tsBlackList = systemService.getById(BlackListEntity.class,
				id
				);
				tsBlackListService.delete(tsBlackList);
				systemService.addLog(message, GlobalConstants.LOG_TYPE_DELETE, GlobalConstants.LOG_LEVEL_INFO);
	...
	@ResponseBody
	public AjaxJson doAdd(BlackListEntity tsBlackList, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		message = "黑名单添加成功";
		try{
	...
	@ResponseBody
	public AjaxJson doUpdate(BlackListEntity tsBlackList, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		message = "黑名单更新成功";
		BlackListEntity t = tsBlackListService.getById(BlackListEntity.class, tsBlackList.getId());
		try {
	...
	public ModelAndView goAdd(BlackListEntity tsBlackList, HttpServletRequest req) {
		if (StringUtil.isNotEmpty(tsBlackList.getId())) {
			tsBlackList = tsBlackListService.getById(BlackListEntity.class, tsBlackList.getId());
	...
	public ModelAndView goUpdate(BlackListEntity tsBlackList, HttpServletRequest req) {
		if (StringUtil.isNotEmpty(tsBlackList.getId())) {
			tsBlackList = tsBlackListService.getById(BlackListEntity.class, tsBlackList.getId());
			req.setAttribute("tsBlackListPage", tsBlackList);
		}
	...
	@ResponseBody
	public ResponseMessage<List<BlackListEntity>> list(HttpServletRequest request, HttpServletResponse response) {
		...
		CriteriaQuery query=new CriteriaQuery(BlackListEntity.class);
		InterfaceUtil.installCriteriaQuery(query, interfaceRuleDto, InterfaceEnum.blacklist_list);
		query.add();
		List<BlackListEntity> listTsBlackLists = this.tsBlackListService.getListByCriteriaQuery(query, false);
	...
	public ResponseMessage<?> get(@PathVariable("id") String id,HttpServletRequest request) {
		...
		BlackListEntity task = tsBlackListService.getById(BlackListEntity.class, id);
	...
	@ResponseBody
	public ResponseMessage<?> create(@RequestBody BlackListEntity tsBlackList, HttpServletRequest request) {
		...
		Set<ConstraintViolation<BlackListEntity>> failures = validator.validate(tsBlackList);
		if (!failures.isEmpty()) {
...
//		}
	...
	@ResponseBody
	public ResponseMessage<?> update(@RequestBody BlackListEntity tsBlackList, HttpServletRequest request) {
	...
	public ResponseMessage<?> delete(@PathVariable("id") String id,HttpServletRequest request) {
		...
		try {
			tsBlackListService.deleteById(BlackListEntity.class, id);
		} catch (Exception e) {
			e.printStackTrace();
